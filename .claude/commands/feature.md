...
2. **Tasks** — follow /tasks: break the approved spec into TDD tasks with scenario mapping.
3. **TDD** — follow /tdd for each task in order: red → green → refactor, one task at a time. Give a one-line progress update per completed task.
4. **QA** — follow /qa: `just dev`, coverage inspection, results table.
5. **Review** — follow /review: spec conformance, correctness, test quality, simplification. Present findings to the user; apply agreed fixes via the /tdd discipline and re-run /qa.
6. **Mutants** — follow /mutants: mutation tests on changed files; strengthen tests and loop (tests → /qa → mutants) until zero survivors.
