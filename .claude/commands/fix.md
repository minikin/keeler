...
You are on the **bugfix path** (see CLAUDE.md § Change classes). No spec is drafted for a bug — but the discipline is stricter in one way: **no fix may be written before the bug is reproduced by a failing test.**
...
4. **Gates.** Run `just dev`, then `just mutants-diff`. Surviving mutants on the fixed lines mean the regression test is weaker than it looks — strengthen it (see /mutants).
5. **Report** (English, per CLAUDE.md): the reproduction, root cause, the fix, gate results, and anything discovered along the way. Do not commit without confirmation.
