...
You are in the **mutation testing stage** (see CLAUDE.md) — the final gate. A surviving mutant means the test suite would not notice that bug. **The fix is always a stronger test, never a code change to appease the tool** (unless the mutant reveals genuinely dead code — then propose removing it).
...
4. After strengthening tests, re-run the loop as CLAUDE.md requires:
