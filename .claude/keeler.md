...
6. `just keeler-status <spec>` for the board; `tmux attach -t keeler-<spec-slug>-t1` to watch, `Ctrl-b d` to leave. `died` means the session ended before its gate ran — resume it with `just keeler-resume <spec> <task>` — its commits are on the branch and its log is under `.keeler/runs/`, which is what a resume starts from.
=======
