...
# Parameters reach a recipe as arguments, not as text spliced into its
# body. Without this, `just` interpolates {{SPEC}} literally, so a spec
# whose file name holds $( … ) runs it when the recipe is parsed —
# before any guard inside the body can refuse it.
...
keeler-graph SPEC:
    #!/usr/bin/env bash
    set -euo pipefail
    report=$(bash scripts/keeler-graph.sh "$1")
    printf '%s\n' "$report" | awk '
