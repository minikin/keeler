...
6. **Gates.** Run `keeler dev`, then `keeler mutants-diff`. Surviving mutants on the fixed lines mean the regression test is weaker than it looks — strengthen it (see /keeler:mutants).
7. **Report** (English, per the rules' § Reporting): the reproduction, root cause, the fix, gate results, and anything discovered along the way. Do not commit without confirmation.
