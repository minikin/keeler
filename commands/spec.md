...
You are in the **spec stage** of that workflow. Your job is analysis and specification only — **do not write any implementation code, do not create tasks yet**.
...
3. **Draft the spec.** Copy the project's `specs/TEMPLATE.md` when that file exists and `${CLAUDE_PLUGIN_ROOT}/templates/spec.md` otherwise, to `specs/NN-<slug>.md` (next free number, kebab-case slug). Fill in:
...
6. **Ask which road, and wait for the answer.** With the spec approved, ask one question — develop this feature **linearly**, or **as a graph**? — and say what each answer does before it is answered. Stop there: nothing below happens until the user has answered.
   ...
   - **graph** — one agent per task, in parallel. Read `${CLAUDE_PLUGIN_ROOT}/graph-mode.md` before acting on this answer. It creates `feat/<spec-slug>` and **commits the approved spec** there, so the answer is the consent for that commit; on any other answer, create nothing and commit nothing.

     Run `keeler keeler-feature-branch <spec>` — it cuts the branch from main, checks it out and commits the spec, or reuses the branch if it is already there. Then invoke /keeler:tasks, and when it has written the graph, hand back with the next steps in order: **commit the graph** /keeler:tasks wrote (fan-out and spawn read the committed spec, never the working tree), then `keeler keeler-fan-out <spec>`.
