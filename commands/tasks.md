...
You are in the **task breakdown stage**. The spec must have **Status: Approved** — if it's still Draft, stop and tell the user to approve it first (via /keeler:spec iteration).
...
2. **Break the work into ordered tasks** (T1, T2, …), each one small enough to be a single red→green→refactor cycle. For every task specify:
   ...
   - which earlier tasks it needs, as `Needs: T1, T4.` — omit it for a root, and use the token once per item: the parser counts it, so do not write the word `Needs:` elsewhere in the same item. This is the dependency graph — read `${CLAUDE_PLUGIN_ROOT}/graph-mode.md` before writing a `Needs:` line, since the plugin's graph parser reads them and every graph recipe builds on what it finds. Two tasks that would edit the **same region** of one file are not independent — give one a `Needs:` on the other, because parallel edits to one region are a merge conflict by construction and the graph is where that is prevented. Files every branch adds to — a recipe in the project's own `Justfile`, a test function, a job in the workflow — merge cleanly if each task adds under its own heading rather than at the end; say where. Hot files with a settled answer: the spec (one line, its own tick), `Cargo.lock` (regenerated on merge), `proptest-regressions/` (append-only seeds), `crap-baseline.json` (never on a branch).
