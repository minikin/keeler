<!-- keeler-version: 0.4.1 -->
# Keeler — workflow rules

The spec-first, test-driven workflow this project follows. They reach you
from the Keeler plugin, whose commands are the stages below and whose
skills load beside them; they exist for Claude Code and no other agent.
...
Two chapters live beside this file, in the plugin root, and are read on
instruction rather than at every session start:

- **`graph-mode.md`** — graph mode: the same pipeline run by one agent per
  task, in parallel. The recipes, the three rules that keep parallel
  branches honest, and a feature start to finish.
- **`gates.md`** — the gate table, the bars, and the baseline discipline.
  **Every gate in it must be green before a feature is done.**

The recipes are `keeler --list`.
...
All feature specs live in `specs/`, written Gherkin style (Given/When/Then). Copy the project's `specs/TEMPLATE.md` when it has one and the plugin's `templates/spec.md` otherwise; number them `NN-slug.md`.
